                ...
                <h3 class="tittle text-center mb-lg-5 mb-3 px-lg-5">NEW 캠핑소식</h3>
	             <div class="info_list">
					...
					<c:if test="${searchCount==null}">
						...
						<div align="right">
							<button type="button" class="btn btn-primary" onclick="location.href='${root}/board/campReview/list.do?pageNumber=${pageNumber}'" >전체목록</button>
						</div>
					</c:if>
					<!--작성글이있다면-->
				 <c:if test="${searchCount>0}">
				  ...
				  <c:if test="${sessionScope.email=='eunsol8287@gmail.com'}">
					<div class="list_buttom" align="right">
						<button type="button" class="btn btn-primary" onclick="location.href='${root}/board/campReview/write.do?user_num=${user_num}'">공지 작성</button>
					</div>
				  </c:if>
					<div align="center">
				 <c:if test="${count>0}">
